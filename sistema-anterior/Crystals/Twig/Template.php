...
abstract class Twig_Template implements Twig_TemplateInterface
{
    ...
    protected function getAttribute($object, $item, array $arguments = array(), $type = Twig_TemplateInterface::ANY_CALL, $isDefinedTest = false, $ignoreStrictCheck = false)
    {
		...
		if($object instanceOf ActiveRecord\Model){
			// if item is a method call them
			if($type == Twig_TemplateInterface::METHOD_CALL){
				return call_user_func_array(array($object, $item), $arguments);
			}
			// else returns an attribute
			return $object->$item; // with this fix all problems for relations or attributes
		}
		
		if(is_object($object) && $type !== Twig_TemplateInterface::METHOD_CALL){
