...
abstract class Connection
{
	...
	static $PDO_OPTIONS = array(
		...
		PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8", // impide que se guarden los datos con simbolos raros
	...
	public function columns($table)
	{
		...
		while (($row = $sth->fetch()))
		{
			...
			// adds values if his type is enum or set
			
			if(preg_match('/^(enum|set)\((.*)\)/', $row['type'], $r)){
				$r[2] = str_replace('\'', '',$r[2]);
				$r[2] = str_replace('"', '',$r[2]);
				$c->length = explode(',', $r[2]);
			}
			$columns[$c->name] = $c;
		}
