...
<div class="panel panel-mint panel-bordered">					
    ...
    <div class="panel-heading">
        <div class="panel-control">
            ...
            <ul class="nav nav-tabs">
                ...
                <li class="{{ i == 1 ? 'active' : '' }}"><a data-toggle="tab" href="#videos_tabs-box-{{ i }}">Bim. {{ i }}</a></li>
                {% endfor %}
            ...
            <div class="btn-group dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-right">
                    <li><a href="#/asignaturas_videos/form?asignatura_id={{ sha1(asignatura.id) }}">{{ icon('register') }} Registrar Nuevo</a></li>
                </ul>
            </div>
            {% endif %}
        </div>
        <h3 class="panel-title">VIDEOTECA</h3>
    </div>
    ...
    <div class="panel-body">
        ...
        <div class="tab-content">
        	{% for i in 1..COLEGIO.total_notas %}
            <div id="videos_tabs-box-{{ i }}" class="tab-pane fade {{ i == 1 ? 'active in' : '' }}">
                {% set videos = asignatura.getVideos(i) %}
                {% if videos|length > 0 %}
                <table class="special">
                	<tr>
                		<th>Nombre</th>
                    	...
                    	<td class="text-center" style="width: 120px">
							...
							<div class="btn-group dropup">
								...
								<ul class="dropdown-menu pull-right" role="menu">
									{% if video.enlace %}
									<li><a href="{{ video.enlace }}" target="_blank">{{ icon('application_get') }} Ver Video</a></li>
									{% endif %}
									<li><a href="#/asignaturas_videos/form/{{ sha1(video.id) }}">{{ icon('register') }} Editar Video</a></li>
                                    <li><a href="javascript:;" onclick="borrar_asignatura_video('{{ sha1(video.id) }}')">{{ icon('delete') }} Borrar Video</a></li>
								</ul>
							</div>
                    		{% else %} <!-- if USUARIO.is('ALUMNO') or USUARIO.is('APODERADO') -->
                            <a class="btn btn-default" href="{{ video.enlace }}" target="_blank">{{ icon('film') }} Ver Video</a>
							{% endif %}
						</td>
                    </tr>
                	{% endfor %}
