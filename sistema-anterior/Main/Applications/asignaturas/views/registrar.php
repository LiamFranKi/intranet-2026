...
$(function(){
	...
	$('#formAsignatura').bootstrapValidator({
		...
		onSuccess: function(e){
			...
			if($('#curso_id').val() == null){
				return zk.pageAlert({message: 'Seleccione al menos un curso.', title: 'Operación Fallida', icon: 'bolt', type: 'danger', container: 'floating'});
			}

			$(_form).sendForm('/asignaturas/save_nuevo', function(r){
	...
	$('#curso_id').attr('multiple', 'true');
	$('#curso_id').find('option[value=""]').remove();
	$('#curso_id').attr('name', 'curso_id[]');
});
</script>
<div id="page-head">
	<div id="page-title">
