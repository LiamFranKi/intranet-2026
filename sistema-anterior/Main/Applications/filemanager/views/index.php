{% if personal.validFileManagerToken(get.token) %}
<script>
var fileManager;
function fillFilesOnTable(files){
	...
	for(i in files){
		...
		data += '<td class="text-center">'+ (file.type == 'File' ? '<input type="checkbox" onclick="fileManager.selectFile(\''+ file.filePath +'\', \''+ file.filename +'\')" '+ (localStorage.getItem(file.filePath) != null ? 'checked' : '') +' />' : '') +'</td>';
		{% endif %}
		data += '<td class="text-center">'+ (file.type == 'Folder' ? '{{ icon('folder') }}' : '{{ icon('page') }}') +'</td>'+
		'<td><a href="'+ (file.type == 'File' ? '/filemanager/download?f=' + file.filePath : 'javascript:;') +'" onclick="'+ (file.type == 'Folder' ? 'fileManager.setBase(\''+ file.folderBase +'\', fillFilesOnTable)' : '') +'"      '+ (file.type == 'Folder' ? '' : 'download') +'>'+ file.filename +'</a></td>'+
		'<td class="text-center">'+ file.size +'</td>'+
		'<td class="text-center">'+ file.modifiedDate +'</td>';
		{% if permissions.DELETE %}
		if(file.filename != '..'){
			data += `<td class="text-center">
                <a href="`+ atob(file.filePath) +`" title="Visualizar" target="_blank">{{ icon('application_form_magnify') }}</a>    
                <a href="javascript:;" title="Borrar Archivo" onclick="fileManager.deleteFile('`+ file.filePath +`')">{{ icon('delete') }}</a>
            </td>`;
		}else{
...
$(function(){
	fileManager = {
		token: '{{ get.token }}',
		base: '{{ base64_encode(personal.getFileManagerDirectory()) }}',
		getFiles: function(callback){
			$('#filesLoading').show();
			$.post('/filemanager/files', {token: fileManager.token, base: fileManager.base, id: '{{ personal.id }}'}, callback, 'json');
		},
			...
			$.prompt({
				state0: {
					...
					submit: function(e, v){
						e.preventDefault();
						if(v){
							$.post('/filemanager/make_folder', {base: fileManager.base, name: $('#fileManagerFolderName').val(), id: '{{ personal.id }}'}, function(r){
								if(parseInt(r[0]) == 1){
									fileManager.getFiles(fillFilesOnTable);
									$.prompt.close();
								}else{
	...
	$('#filesHandler').fileupload({
        ...
        add: function(e, data){
			data.formData = {base: fileManager.base, id: '{{ personal.id }}'}
			data.submit();
			$('#uploader').show();
		},
...
<div class="modal-menu bg-primary">
	{% if get.selectFiles == "true" and not get.backMySpace == "true" %}
	...
	{% endif %}
	{% if permissions.PUBLIC and publicPersonal %}
	<button class="btn-default btn" onclick="fancybox('/filemanager/index/{{ publicPersonal.id }}?token={{ publicPersonal.getFileManagerToken() }}&p={{ base64_encode('R,U') }}&base=/&backMySpace=true&selectFiles={{ get.selectFiles }}')">{{ icon('pictures') }} Espacio Público</button>
	{% endif %}
	{% if COLEGIO.getArchivosUsuario(USUARIO.tipo) %}
	<a class="btn-default btn" href="{{ COLEGIO.getArchivosUsuario(USUARIO.tipo) }}" target="_blank">{{ icon('folder_go') }} Otros Archivos</a>
	{% endif %}
</div>

{% if permissions.UPLOAD %}
<div class="modal-content" id="uploader" style="display: none; width: 800px">
	<div class="modal-header">
		<h3 class="modal-title">Subiendo Archivos</h3>
	</div>
...
<div class="modal-content" style="width: 800px;">
	<div class="modal-header">
		<h3 class="modal-title">Administrador de Archivos</h3>
	</div>
