<?php
class Asignaturas_enlacesApplication extends Core\Application{
	...
	function index($r){
		$asignaturas_enlaces = Asignatura_Enlace::all();
		$this->render(array('asignaturas_enlaces' => $asignaturas_enlaces));
	}
	
	function save(){
		$r = -5;
		$this->asignatura_enlace->set_attributes(array(
		...
		if($this->asignatura_enlace->is_valid()){
			$r = $this->asignatura_enlace->save() ? 1 : 0;
		}
		echo json_encode(array($r, 'id' => $this->asignatura_enlace->id, 'errors' => $this->asignatura_enlace->errors->get_all()));
	}

	function borrar($r){
		$r = $this->asignatura_enlace->delete() ? 1 : 0;
		echo json_encode(array($r));
	}
	
	function __getObjectAndForm(){
		$this->set('__active', 'asignaturas_enlaces', true);
		$this->asignatura_enlace = !empty($this->params->id) ? Asignatura_Enlace::find(['conditions' => 'sha1(id) = "'.$this->params->id.'"']) : new Asignatura_Enlace();
		$this->context('asignatura_enlace', $this->asignatura_enlace); // set to template
		...
		if(in_array($this->params->Action, array('form'))){
			$this->form = $this->__getForm($this->asignatura_enlace);
