<?php
class Tarea_Prueba extends ActiveRecord\Model{
	
	static $pk = 'id';
	static $table_name = 'tareas_pruebas';
	static $connection = '';
	...
	function calificar(){
		...
		$preguntas = $this->tarea->getPreguntas();
		$resultados = array();

		foreach($preguntas As $pregunta){
			if($pregunta->tipo_respuesta != 'ALTERNATIVAS') continue;
			
			if($this->respuestaCorrecta($pregunta)){
				$puntaje += $tarea->puntos_correcta;
				++$correctas;
			}else{
